...
def test_build_app_increments_session_number_on_resume(tmp_path: Path) -> None:
    ...
    build_state = cast(
        Callable[[str, dict[str, object]], dict[str, Any]], getattr(app, "_build_play_state")
    )
...
def test_build_app_scrollback_excludes_retconned_transcript(tmp_path: Path) -> None:
    root = tmp_path / "retcon-scrollback"
    manifest = init_campaign(name="Retcon Scrollback", root=root, provider="fake")
    conn = open_campaign_db(root / "campaign.sqlite")
    try:
        turns = TurnRecordRepository(conn)
        transcripts = TranscriptRepository(conn)
        for turn_id, status, content in [
            ("turn_000001", "complete", "Canonical narration remains."),
            ("turn_000002", "retconned", "Removed narration should not resume."),
        ]:
            turns.upsert(
                TurnRecord(
                    turn_id=turn_id,
    ...
    try:
        assert "Canonical narration remains." in app.initial_scrollback
        assert "Removed narration should not resume." not in app.initial_scrollback
    finally:
        app.on_unmount()

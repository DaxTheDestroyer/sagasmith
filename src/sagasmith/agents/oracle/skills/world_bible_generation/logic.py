"""World bible generation skill logic for the Oracle agent."""
...
from sagasmith.prompts.oracle import world_bible_generation as prompt
...
DEFAULT_WORLDGEN_MODEL = "fake-default"
DEFAULT_CHEAP_MODEL = "fake-cheap"
WORLD_BIBLE_MAX_TOKENS = 1800
...
def generate_world_bible(
    *,
    ...
    house_rules: HouseRules | dict[str, Any],
    ...
    """Generate and validate the hidden `WorldBible` with structured JSON output."""

    profile = PlayerProfile.model_validate(player_profile)
    policy = ContentPolicy.model_validate(content_policy)
    rules = HouseRules.model_validate(house_rules)
    user_prompt = prompt.build_user_prompt(profile, policy, rules)
    request = LLMRequest(
        agent_name="oracle.world-bible-generation",
        model=model,
        messages=[
        ...
        temperature=0.7,
        max_tokens=WORLD_BIBLE_MAX_TOKENS,
        timeout_seconds=90,
        metadata={"prompt_version": prompt.PROMPT_VERSION, "skill": "world-bible-generation"},
    )
    _preflight_budget(
        client=llm_client,
        request=request,
    ...
    return WorldBible.model_validate(response.parsed_json)

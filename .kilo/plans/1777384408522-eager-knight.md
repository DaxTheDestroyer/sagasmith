# Phase 6: AI GM Story Loop — Discussion Plan

**Workflow:** discuss-phase 6
**Date:** 2026-04-28
**Output target:** `.planning/phases/06-ai-gm-story-loop/06-CONTEXT.md`

## Decisions Captured

All four gray areas discussed and resolved. Full CONTEXT.md content below.
...
- **D-03:** Pre-generation gate (SAFE-01/SAFE-02): Oracle's `content-policy-routing` skill uses keyword/pattern matching against `ContentPolicy.hard_limits` and `soft_limits` string lists to block or reroute scene beats before Orator runs. This is fast, zero extra LLM cost, and consistent with the existing `RedactionCanary` pattern-match model.
- **D-04:** Post-generation gate (SAFE-03): After Orator accumulates prose, a cheap LLM classifier call scans the text against `ContentPolicy` before the narration is written to `pending_narration`. On violation: request one rewrite attempt; if second attempt also fails, fall back to a safe terse fallback line. Log every event via `SafetyEventService.log_fallback()`.
- **D-05:** The cheap LLM classifier call for the post-gate uses the configured `cheap_model` (already defined in LLM config), counts against the session budget via CostGovernor, and is logged with `llm-call-logging`.
- **D-06:** QA-05 is implemented using `DeterministicFakeClient` scripted to return hard-limit trigger content; test asserts those strings do not appear in `pending_narration` after the safety post-gate runs.
...
- **D-15:** For Phase 6, Oracle and Orator receive a minimal stub `MemoryPacket` constructed by the runtime/bootstrap layer: empty `entities`, empty `callbacks`, minimal transcript context (last 3 transcript entries from SQLite), within token cap. Full Archivist memory assembly (AI-11) remains Phase 7 scope.
...
- Exact prompt templates for Oracle scene-brief-composition and Orator scene-rendering — planner decides
- Exact `WorldBible` and `CampaignSeed` Pydantic model field set (add to schemas/ as needed)
- Whether the Oracle bypass detection is regex/keyword or a lightweight LLM call
...
- `docs/specs/GAME_SPEC.md` §4 — Turn flow: mechanics-first observable behavior, late-roll exception.
...
- `src/sagasmith/agents/archivist/skills/memory-packet-assembly/SKILL.md` — read to understand stub boundary
...
- `pre_narration` checkpoint is written before Orator runs — deterministic outcomes locked in at that point (GRAPH-07 guaranteed structurally).
...
### Integration Points
- `src/sagasmith/graph/graph.py` — routing edges need a new conditional: `scene_brief` present + beats not resolved → skip oracle_node. New `world_bible` and `campaign_seed` fields may be added to `SagaGraphState`.
- `src/sagasmith/graph/routing.py` — exhaustiveness guard must be updated when new routing branches are added.
- `src/sagasmith/agents/oracle/node.py` — stub comment explicitly says "Phase 6 replaces". Drop-in replacement site.
- `src/sagasmith/agents/orator/node.py` — stub comment explicitly says "Phase 6 replaces". Drop-in replacement site.
- `src/sagasmith/agents/rules_lawyer/node.py` — Phase 5 wired deterministic services. Phase 6 adds the LLM intent-to-proposal layer upstream of existing deterministic calls.
- `src/sagasmith/app/tui.py` (or equivalent Textual shell) — reads `pending_narration` from completed graph state and appends to NarrationArea; no changes to streaming delivery contract in Phase 6.
...
- Safety post-gate uses the cheap LLM classifier (cheap_model in config) — keep costs bounded per turn.
...
- World bible persistence to vault — Phase 7.
